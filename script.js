		...
		pdata.forEach(function(batt, i) {
			if(typeof(batt.opp) !== "undefined") {
				if(batt.head === "Wild-Pokemon") {
					$('#bside').append('<button	id="batt' + i + '" class="popover-maps btn btn-default battler bg-' + toLower(batt.head) + '" data-toggle="popover" style="background-image:url(images/avatar/' + batt.head + '.png);background-repeat: no-repeat; background-position: 100% 0%;">' + '<b>' + batt.opp + '</b><br>' + '<small>(' + batt.head + ')</small><br>' + 'Exp: ' + numberWithCommas(batt.exp) + '</button>');
					...
					$('#bside').append('<button	id="batt' + i + '" class="popover-maps btn btn-default battler bg-' + toLower(batt.head) + '" data-toggle="popover" style="background-image:url(images/avatar/' + compbattle + '.png);background-repeat: no-repeat; background-position: 100% 0%;">' + '<b>' + batt.opp + '</b><br>' + '<small>(' + batt.head + ')</small><br>' + 'Exp: ' + numberWithCommas(batt.exp) + '</button>');
					$('.popover-maps').popover({
						trigger: 'focus',
						html: true,
						placement: 'top',
					...
					$('#bside').append('<button id="batt' + i + '" class="popover-dismiss btn btn-default battler bg-' + toLower(batt.head) + '"  data-toggle="popover"  style="background-image:url(images/avatar/' + batt.opp + '.png);background-repeat: no-repeat; background-position: 100% 0%;"> ' + '<b>' + batt.opp + '</b><br>' + '<small>(' + batt.head + ')</small><br>' + 'Exp: ' + numberWithCommas(batt.exp) + '</button>');
...
function createAlert(type, message) {
	...
	var alert = '' + '<div class="alert alert-' + toLower(aclass) + ' alert-dismissible fade in" role="alert">' + '<button class="close" aria-label="Close" data-dismiss="alert" type="button">' + '<span aria-hidden="true">×</span>' + '</button>' + '<strong>' + ucfirst(type) + '!</strong> ' + message + '</div>';
	removeAlert();
...
function addBattleEntry(bid, opp, exp, head) {
	if($('#bt_' + bid).length) {
		...
		var html = '<tr class="btitem bg-' + toLower(head) + '" id="bt_' + bid + '">' + '<td class="name">' + opp + '</td>' + '<td class="name" style="text-align:center; font-size:12px;">' + head + '</td>' + '<td class="battles" style="text-align:center">1</td>' + '<td class="exp" data-num="' + exp + '" style="text-align:center">' + numberWithCommas(exp) + '</td>' + '<td class="total" data-num="' + exp + '" style="text-align:center">' + numberWithCommas(exp) + '</td>' + '<td class="btclose"></td>' + '</tr>';
